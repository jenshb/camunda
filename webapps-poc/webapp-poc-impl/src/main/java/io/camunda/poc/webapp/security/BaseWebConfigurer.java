...
import static io.camunda.poc.webapp.security.WebAppsPocURIs.COOKIE_JSESSIONID;
import static io.camunda.poc.webapp.security.WebAppsPocURIs.LOGIN_RESOURCE;
import static io.camunda.poc.webapp.security.WebAppsPocURIs.LOGOUT_RESOURCE;
import static io.camunda.poc.webapp.security.WebAppsPocURIs.RESPONSE_CHARACTER_ENCODING;
import static io.camunda.poc.webapp.security.WebAppsPocURIs.X_CSRF_TOKEN;
...
public abstract class BaseWebConfigurer {
  ...
  @Autowired protected WebAppsPocProperties webAppsPocProperties;

  @Autowired WebAppPocProfileService errorMessageService;
  final CookieCsrfTokenRepository cookieCsrfTokenRepository = new CookieCsrfTokenRepository();
  @Autowired private WebAppPocProfileService profileService;
  ...
  protected void applySecurityHeadersSettings(final HttpSecurity http) throws Exception {
    http.headers()
        .contentSecurityPolicy(
            webAppsPocProperties.getSecurityProperties().getContentSecurityPolicy());
  }
  ...
  private void defaultFilterSettings(
      final HttpSecurity http, final HandlerMappingIntrospector introspector) throws Exception {
    if (webAppsPocProperties.isCsrfPreventionEnabled()) {
    ...
    http.authorizeRequests(
            (authorize) -> {
              authorize
                  .requestMatchers(WebAppsPocURIs.getAuthWhitelist(introspector))
                  .permitAll()
                  /*.requestMatchers(
