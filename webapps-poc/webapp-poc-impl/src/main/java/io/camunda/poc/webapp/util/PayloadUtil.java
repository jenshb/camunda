...
public class PayloadUtil {
  ...
  @Autowired
  @Qualifier("webAppsPocObjectMapper")
  ...
  public String readJSONStringFromClasspath(final String filename) {
    try (final InputStream inputStream = PayloadUtil.class.getResourceAsStream(filename)) {
      if (inputStream != null) {
        ...
        throw new WebAppPocRuntimeException("Failed to find " + filename + " in classpath ");
      }
    } catch (final IOException e) {
      throw new WebAppPocRuntimeException(
          "Failed to load file " + filename + " from classpath ", e);
    }
  }
