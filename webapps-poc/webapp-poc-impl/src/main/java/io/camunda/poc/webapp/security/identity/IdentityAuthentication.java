 ...
 */
package io.camunda.poc.webapp.security.identity;

import static io.camunda.poc.webapp.security.tenant.TenantAwareAuthentication.TENANT_NAMES_COMPARATOR;
...
import io.camunda.poc.webapp.security.OldUsernameAware;
import io.camunda.poc.webapp.security.Permission;
...
public class IdentityAuthentication extends AbstractAuthenticationToken
  ...
  private volatile List<WebAppsPocTenant> tenants = Collections.emptyList();
  ...
  private void retrieveTenants() {
    if (getWebAppsPocProperties().getMultiTenancy().isEnabled()) {
      try {
  ...
  public void authenticate(final Tokens tokens) {
    ...
    try {
      final WebAppsPocProperties props = getWebAppsPocProperties();
  ...
  @NotNull
  private static WebAppsPocProperties getWebAppsPocProperties() {
    return SpringContextHolder.getBean(WebAppsPocProperties.class);
  }
