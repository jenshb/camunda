...
import io.camunda.poc.webapp.rest.exception.APIException;
import io.camunda.poc.webapp.rest.exception.Error;
import io.camunda.poc.webapp.rest.exception.ForbiddenActionException;
import io.camunda.poc.webapp.rest.exception.NotFoundApiException;
import io.camunda.poc.webapp.security.WebAppPocProfileService;
...
public abstract class InternalAPIErrorController {
  ...
  @Autowired private WebAppPocProfileService webAppPocProfileService;

  @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
  @ExceptionHandler(WebAppPocRuntimeException.class)
  public ResponseEntity<Error> handleException(final WebAppPocRuntimeException exception) {
    LOGGER.warn(exception.getMessage(), exception);
    final Error error =
        new Error()
            .setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value())
            .setMessage(webAppPocProfileService.getMessageByProfileFor(exception));
  ...
  public ResponseEntity<Error> handleAccessDeniedException(
      final ForbiddenActionException exception) {
    LOGGER.warn(exception.getMessage(), exception);
    final Error error =
        new Error()
